...
class Client():
    ...
    def set_attribute(self, app, key, value):
        ...
        :raises: ResponseError in case an HTTP error status was returned
        """
        path = 'setattribute/' + urllib.parse.quote(app) + '/' + urllib.parse.quote(key)
        res = self.__make_ocs_request(
                'POST',
                self.OCS_SERVICE_PRIVATEDATA,
                path,
    ...
    def delete_attribute(self, app, key):
        ...
        :raises: ResponseError in case an HTTP error status was returned
        """
        path = 'deleteattribute/' + urllib.parse.quote(app) + '/' + urllib.parse.quote(key)
        res = self.__make_ocs_request(
                'POST',
                self.OCS_SERVICE_PRIVATEDATA,
                path
    ...
    def __make_ocs_request(self, method, service, action, **kwargs):
        ...
        if self.__debug:
            print(('OCS request: ' +method+' '+ self.url + path))
